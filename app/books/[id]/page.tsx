...
import {
  SANITY_ALL_BOOK_PAGES_QUERY,
  SANITY_BOOK_PAGE_QUERY,
  SANITY_BOOK_PAGE_SEO_QUERY,
} from './builder/query';
import { notFound } from 'next/navigation';
import { PageParams } from '@/lib/types';
import { ISanityBookPageQueryResponse } from './builder/interface';

const REVALIDATION_TIME = 3600;

export default async function Page({ params }: PageParams) {
  const sanityData = await sanityFetch<ISanityBookPageQueryResponse>({
    query: SANITY_BOOK_PAGE_QUERY,
    params: {
      slug: params.id,
    },
...
export async function generateMetadata({
  ...
  const data = await sanityFetch<ISanitySeoData>({
    query: SANITY_BOOK_PAGE_SEO_QUERY,
    params: {
      slug: params.id,
    },
    revalidate: REVALIDATION_TIME,
  ...
  return extractMetadata({ data, docType: 'book' });
}

// Get all page routes
export async function generateStaticParams() {
  const docs = await sanityFetch<ISanityLimitedSeoData[]>({
    query: SANITY_ALL_BOOK_PAGES_QUERY,
    disablePreview: true,
    revalidate: REVALIDATION_TIME,
  });
